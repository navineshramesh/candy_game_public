...
class Input {
  ...
  get jumpTap(){return this.p.Space||this.p.ArrowUp||this.p.KeyW||this.p.KeyB;}
  get jumpHold(){return this.d.Space||this.d.ArrowUp||this.d.KeyW||this.d.KeyB;}
...
class Particle {
  ...
  draw(ctx){
    if (this.life <= 0) return; // Ensure particle is still alive
    const s=M.lerp(this.sz,this.sz2,this.tp);if(s<=0)return;
    ctx.save();ctx.globalAlpha=this.al;
    ctx.translate(this.x,this.y);ctx.rotate(this.rot);
    
    // Optimized Glow: shadowBlur is too slow. 
    // We'll use a larger, faint circle instead for the 'glow' effect.
    if(this.glow) {
      ctx.fillStyle = this.col;
      ctx.globalAlpha = this.al * 0.3;
...
class Particles {
  ...
  hit(x,y,col='#FFD700',glow=false){ // Default glow to false for hit particles
    this._add(x,y,10,{type:'circle',col,sz:4,sz2:0,smin:80,smax:260,
      life:.35,gr:500,fr:.93,ang:-Math.PI/2,spread:Math.PI*.9,glow});
    this._add(x,y,4,{type:'star',col:'#fff',sz:3,sz2:0,smin:50,smax:130,life:.2,gr:200,glow});
  }
  candy(x,y,n=8){
  ...
  explosion(x,y,sc=1,glow=false){ // Disable glow for performance
    ...
    for(let i=0;i<28*sc;i++)
      this._add(x,y,1,{
        ...
        smin:80,smax:360*sc,life:M.rand(.5,1.4),gr:250,fr:.95,glow,
      });
  ...
  shockwave(x,y,col='#88ff88'){
    for(let r=0;r<3;r++)
      this.list.push(new Particle(x,y,{type:'ring',col,sz:5+r*4,sz2:90+r*24,
        vx:0,vy:0,life:.55,gr:0,fr:1})); // Reduced explosion intensity from 2 to 1.2
    this._add(x,y,14,{col,sz:5,sz2:0,smin:50,smax:200,life:.6,gr:0,fr:.96,glow:true});
  }
  update(dt){
    this.list=this.list.filter(p=>p.update(dt));
    if (this.list.length > 300) this.list.splice(0, this.list.length - 300); // Hard cap for particles
  }
...
const DRAW = {
  ...
  utkarsh(ctx,x,y,facing,phase,af,t,flash=false){
    ctx.save();ctx.translate(~~x,~~y);if(facing<0)ctx.scale(-1,1);
    ...
    ctx.fillStyle=flash?'#fff':rg;ctx.beginPath();
    ...
    ctx.strokeStyle=flash?'#fff':'#ffd700';ctx.lineWidth=2.5;ctx.stroke();
    const rcols=['#ff6b6b','#ffe66d','#4ecdc4','#ff8b94','#c77dff','#48dbfb'];
    [[-20,30,0],[-10,50,1],[10,45,2],[20,30,3],[-5,60,4],[5,55,5]].forEach(([rx,ry,ci])=>{
      ctx.fillStyle=flash?'#fff':rcols[ci];ctx.beginPath();ctx.ellipse(rx,ry+bob,4,6,0,0,Math.PI*2);ctx.fill();
      if(!flash){ctx.fillStyle='rgba(255,255,255,.4)';ctx.fillRect(rx-4,ry-8+bob,3,5);}
    });
    // body core
    const bg2=ctx.createLinearGradient(-25,-10,25,30);bg2.addColorStop(0,'#6600cc');bg2.addColorStop(1,'#3d0080');
    ctx.fillStyle=flash?'#fff':bg2;ctx.beginPath();ctx.ellipse(0,0+bob,24,30,0,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle=flash?'#fff':'#ffd700';ctx.lineWidth=2;ctx.stroke();
    ctx.fillStyle=flash?'#fff':'#ffd700';star(ctx,0,2+bob,14,7,5);ctx.fill();
    ctx.fillStyle=flash?'#fff':'#ff1493';star(ctx,0,2+bob,7,3.5,5);ctx.fill();
    // left arm
    ctx.strokeStyle=flash?'#fff':'#5500aa';ctx.lineWidth=18;ctx.lineCap='round';
    ctx.beginPath();ctx.moveTo(-26,-4+bob);ctx.lineTo(-44,22+bob);ctx.stroke();
    ctx.fillStyle=flash?'#fff':'#6a28aa';ctx.beginPath();ctx.ellipse(-44,26+bob,11,9,0,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle=flash?'#fff':'#6a28aa';ctx.lineWidth=5;
    ...
    ctx.strokeStyle=flash?'#fff':'#5500aa';ctx.lineWidth=18;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(16,26);ctx.stroke();
    ctx.fillStyle=flash?'#fff':'#6a28aa';ctx.beginPath();ctx.ellipse(16,30,11,9,0,0,Math.PI*2);ctx.fill();
    ctx.save();ctx.translate(16,30);
    ctx.strokeStyle=flash?'#fff':'#8844cc';ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(8,-50);ctx.stroke();
    ctx.strokeStyle=flash?'#fff':'#ffd700';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(8,-50);ctx.stroke();
    ctx.save();ctx.translate(8,-50);
    ctx.strokeStyle=flash?'#fff':'#fff';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(0,-8);ctx.stroke();
    ...
    ctx.fillStyle=flash?'#fff':lpg;ctx.beginPath();ctx.arc(0,-20,20,0,Math.PI*2);ctx.fill();
    ctx.restore();ctx.restore();ctx.restore();
    // pauldrons
    [-28,28].forEach((px,i)=>{
      ctx.fillStyle=flash?'#fff':'#4b0082';ctx.beginPath();ctx.ellipse(px,-12+bob,16,11,i===0?-.4:.4,0,Math.PI*2);ctx.fill();
      ctx.strokeStyle=flash?'#fff':'#ffd700';ctx.lineWidth=2;ctx.stroke();
      const gc2=['#ff6b6b','#4ecdc4'][i];ctx.fillStyle=flash?'#fff':gc2;
      ctx.beginPath();ctx.arc(px,-12+bob,5,0,Math.PI*2);ctx.fill();
    });
    // neck
    ctx.fillStyle=flash?'#fff':'#5500aa';ctx.fillRect(-10,-36+bob,20,10);
    // head
    const hg=ctx.createRadialGradient(-4,-52+bob,5,0,-50+bob,20);hg.addColorStop(0,'#8844cc');hg.addColorStop(1,'#4b0082');
    ctx.fillStyle=flash?'#fff':hg;ctx.beginPath();ctx.ellipse(0,-50+bob,20,18,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle=flash?'#fff':'#ffd700';ctx.lineWidth=1.5;ctx.stroke();
    // crown base
    ctx.fillStyle=flash?'#fff':'#ffd700';ctx.beginPath();ctx.moveTo(-32,-62+bob);ctx.lineTo(-32,-68+bob);ctx.lineTo(32,-68+bob);ctx.lineTo(32,-62+bob);ctx.closePath();ctx.fill();ctx.strokeStyle=flash?'#fff':'#ffaa00';ctx.lineWidth=2;ctx.stroke();
    // crown spikes
    ...
    cpts.forEach(({x:cx2,h,col,tp})=>{
      ...
      if(tp==='lollipop'){ctx.strokeStyle=flash?'#fff':col;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(cx2,-68+bob);ctx.lineTo(cx2,-68-h+5+bob);ctx.stroke();ctx.beginPath();ctx.arc(cx2,-68-h+bob,6,0,Math.PI*2);ctx.fill();}
    ...
    [0,72,144,216,288].forEach((deg,i)=>{
      ...
      ctx.fillStyle=flash?'#fff':gc3;
      ctx.beginPath();ctx.arc(Math.cos(a2)*14,-65+Math.sin(a2)*5+bob,3,0,Math.PI*2);ctx.fill();
    });
    // eyes
    const ec=flash?'#fff':enraged?'#ff0000':'#ffd700';
    ctx.fillStyle=ec;
    [-7,7].forEach(ex=>{
      ctx.beginPath();ctx.ellipse(ex,-50+bob,6,7,0,0,Math.PI*2);ctx.fill();
      ctx.fillStyle=flash?'#fff':'#000';ctx.beginPath();ctx.arc(ex,-50+bob,3.5,0,Math.PI*2);ctx.fill();
      if(!flash){ctx.fillStyle='rgba(255,255,255,.6)';ctx.beginPath();ctx.arc(ex-2,-53+bob,1.5,0,Math.PI*2);ctx.fill();}
      ctx.fillStyle=ec;
    });
    ctx.strokeStyle=flash?'#fff':enraged?'#ff4400':'#ffd700';ctx.lineWidth=2.5;
    ctx.beginPath();ctx.moveTo(-16,-56+bob);ctx.lineTo(-4,-52+bob);ctx.stroke();
    ctx.beginPath();ctx.moveTo(16,-56+bob);ctx.lineTo(4,-52+bob);ctx.stroke();
    ctx.strokeStyle=flash?'#fff':'#ffd700';ctx.lineWidth=2;
    if(enraged){ctx.beginPath();ctx.arc(0,-42+bob,10,.4,Math.PI-.4);ctx.stroke();ctx.fillStyle=flash?'#fff':'#2a0050';ctx.fill();}
    ...
    if(desp){for(let i=0;i<8;i++){const a2=i/8*Math.PI*2+t*3,r2=65+Math.sin(t*3+i)*12;
      ...
      ctx.fillStyle=flash?'#fff':dc;
      ctx.beginPath();ctx.arc(Math.cos(a2)*r2,Math.sin(a2)*r2*.6+bob,4+Math.sin(t*2+i)*2,0,Math.PI*2);ctx.fill();}}
    ctx.restore();
  },
...
class Player {
  ...
  update(inp,dt,plats,projectiles,parts,audio,cam){
    ...
    if(inp.attackTap&&this.attackCd<=0){
      this.attackCd=1/(1+this.upgrades.atk*.25);
...
class Enemy {
  ...
  takeDmg(dmg,cam,audio,parts){
    ...
    this.hitPause=CFG.HITPAUSE;
    this.hurtTimer=0.18;
    parts.hit(this.cx,this.cy,'#FF6B6B', false); // Explicitly disable particle glow
...
class Usman extends Enemy {
  constructor(x,y){
    super(x,y,{w:60,h:72,hp:280,name:'USMAN',type:'usman',isBoss:true,candy:10}); // Changed candy to 3
  ...
  draw(ctx,t){
    if(this.dead) ctx.globalAlpha = Math.max(0, this.dying);
    if(this.hurtTimer>0){ctx.filter='brightness(3)';}
    
    // Only pass current phase if boss is alive to prevent "candy loop" effects on death
    const drawPhase = (this.hp > 0) ? this.phase : 0;
    DRAW.usman(ctx,this.cx,this.y+this.h,this.facing,drawPhase,this.af,t);
    ctx.filter='none';
    ctx.globalAlpha=1;
...
class Noah extends Enemy {
  constructor(x,y){
    super(x,y,{w:70,h:90,hp:360,name:'NOAH',type:'noah',isBoss:true,candy:60}); // Candy is already 50
  ...
  draw(ctx,t){
    if(this.hurtTimer>0)ctx.filter='brightness(3)';
    const drawPhase = (this.hp > 0) ? this.phase : 0;
    DRAW.noah(ctx,this.cx,this.y+this.h,this.facing,drawPhase,this.af,t);
    ctx.filter='none';
...
class Utkarsh extends Enemy {
  ...
  update(dt,player,plats,projectiles,parts,audio,cam,guards){
    ...
    if(this.hp<this.maxHp*.66&&this.phase<2){
      ...
      parts.explosion(this.cx,this.cy,1.2); // Reduced explosion intensity
      parts.explosion(this.cx,this.cy,1); 
    }
  ...
  draw(ctx,t){
    // laser beam
    if(this.laserActive>0 && this.hp > 0){ // Combined conditions and fixed unclosed if
    ...
    // High-performance white flash silhouette
    if(this.dead) ctx.globalAlpha = Math.max(0, this.dying);
    
    // Pass phase 0 if dead to stop the "rolling candy loop" (halo/gems)
    const flash = this.hurtTimer > 0;
    DRAW.utkarsh(ctx,this.cx,this.y+this.h,this.facing,this.phase,this.af,t,flash);
    const drawPhase = (this.hp > 0) ? this.phase : 0;
    DRAW.utkarsh(ctx,this.cx,this.y+this.h,this.facing,drawPhase,this.af,t,flash);
    ctx.globalAlpha = 1;
  ...
  update(inp,dt){
    ...
    if(inp.interact){
      ...
      if(this.player.candy>=it.cost&&(this.player.upgrades[it.key]||0)<3){
        ...
        this.msg=`Bought: ${it.name}!`;this.msgTimer=1.5;this.audio.buySound(); // Removed saveProgress() call
  ...
  update(dt){
    ...
    if(this.phase==='fade_in'){
      ...
      if(musicTime>=BEAT_DROP&&!this.beatPlayed){
        ...
        this.parts.explosion(this.bossX,this.bossY,1.2); // Reduced explosion intensity
        this.audio.roar();
      }
      ...
      if(this.t>0.8){
        this.phase = 'end'; // Prevent multiple calls to onDone
        this.onDone();
      }
    }
  }
...
// ══════════════════════════════════════════════════════════════════════
class VictoryScene {
  constructor(audio,onDone,title='VICTORY!',sub='The Candy Kingdom is yours!',msg='Your journey continues...'){
    this.audio=audio;this.onDone=onDone;
    this.t=0;this.parts=new Particles();this.fadeAlpha=1;
    this.title=title;this.sub=sub;this.msg=msg;
  ...
  update(dt){
    ...
    if(this.t>2&&inp&&inp.p&&Object.keys(inp.p).length>0)this.onDone();
  }
  draw(ctx){
    // dark purple bg
    ...
    ctx.font='bold 72px monospace';ctx.textAlign='center';ctx.fillText(this.title,0,0);
    ctx.shadowBlur=0;ctx.restore();
    // subtitle
    ctx.fillStyle='#ffd700';ctx.font='bold 24px monospace';ctx.textAlign='center';
    ctx.fillText(this.sub,CFG.W/2,CFG.H/2);
    // subtext
    ctx.fillStyle='rgba(255,255,255,.6)';ctx.font='16px monospace';
    ctx.fillText(this.msg,CFG.W/2,CFG.H/2+40);
    if(this.t>3){
...
class SettingsScene {
  ...
  draw(ctx){
    ...
    const ctrls=[['Move','WASD / Arrow Keys'],['Jump','Space / W / ↑ / B'],['Nail Attack','J'],['Shoot','K'],['Dash','L'],['Interact/Shop','E']];
    ctrls.forEach(([a,b],i)=>{
...
class Game {
  ...
  levelCompleteTrigger = false; // Flag to synchronously trigger level completion
  ...
  loadLevel(idx){
    ...
    this.bossDefeated=false; // Reset boss defeated flag
    this.bossCountdown=0;
    ...
    this.scene = idx >= 1 && idx <= 3 ? 'boss' : 'game'; // Set scene based on level index, Utkarsh has a cutscene
  ...
  loop(ts){
    ...
    else if(this.scene==='settings'||this.scene==='controls'){ // Controls scene also uses SettingsScene
  ...
  updateGame(dt,inp){
    ...
    if (boss && boss.hp <= 0 && !this.bossDefeated) {
      ...
      boss.dying = 1; // Start boss dying animation
      boss.phase = 0; // Immediate reset to stop FX loops
      pl.candy += boss.candy; // award candy immediately // Removed saveProgress() call
      this.parts.explosion(boss.cx, boss.cy, 1.2); // Reduced explosion intensity 
      this.aud.victory();this.cam.shake(18,0.8); // Immediately set trigger for level completion
      this.levelCompleteTrigger = true;
      this.levelComplete()
    } 
    ...
    if(this.levelIndex===0){
      ...
      if(pl.x>2000&&this.guards.length===0&&!this.transitioning){
  ...
  levelComplete(){
    ...
    if (!this.transitioning) { // Prevent multiple transitions
      this.startTransition(() => { this.cutscene = new Cutscene(this.aud, () => { this.startTransition(() => { this.loadLevel(4); }); }); this.scene = 'cutscene'; });
    }
